...
impl AmmStorage {
  fn load_from_amm(data: &[u8]) -> Result<Composition, String> {
    let json = core::str::from_utf8(data).map_err(|err| err.to_string())?;
    Composition::deserialize_json(json)
  }

  fn save_to_amm(composition: &Composition) -> String {
